		...
		BE::State: StateBackend<BlakeTwo256>,
{
	...
	let max_duration = time::Duration::from_secs(max_logs_request_duration);
		...
		CT: fp_rpc::ConvertTransaction<<B as BlockT>::Extrinsic> + Send + Sync + 'static,
{
	...
	fn estimate_gas(
		...
		Box::pin(async move {
			...
			#[rustfmt::skip]
				let executable = move |request, gas_limit, api_version, api: sp_api::ApiRef<'_, C::Api>, estimate_mode| -> Result<ExecutableResult> {
				let CallRequest {
					from,
					to,
					gas,
	...
	fn logs(&self, filter: Filter) -> BoxFuture<Result<Vec<Log>>> {
		...
		let max_logs_request_duration = self.max_logs_request_duration;
